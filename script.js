;(function (window, document) {
  ...
  const utils = {
    ...
    on: (el, evt, fn, opts) => el && el.addEventListener(evt, fn, opts || false),
    once: (el, evt, fn, opts) => el && el.addEventListener(evt, fn, Object.assign({ once: true }, opts)),
    off: (el, evt, fn, opts) => el && el.removeEventListener(evt, fn, opts || false),
  }
...
ThemeManager.init()
  ...
  function initPrueba() {
    ...
    function soltado(e) {
      ...
      // Determinar el contenedor donde se soltó
      let contenedor
      ...
      if (contador === 3) {
        const img1Ok = utils.qs('#cajasoltar>img')?.getAttribute('src')?.split('/')?.includes('rompe1.png')
        const img2Ok = utils.qs('#cajasoltar2>img')?.getAttribute('src')?.split('/')?.includes('rompe2.png')
        const img3Ok = utils.qs('#cajasoltar3>img')?.getAttribute('src')?.split('/')?.includes('rompe3.png')
        ...
        if (img1Ok && img2Ok && img3Ok) {
          ...
          setTimeout(() => {
            ...
            if (titulo) {
              titulo.innerHTML = '<span>¡Felicitaciones!<br>Puzzle correctamente resuelto</span>'
              titulo.style = 'animation:feliz 3s forwards;position:relative'
            }
        ...
        // Si el puzzle está incorrecto
          ...
          setTimeout(() => {
            ...
            const titulo = utils.qs('.espacio-titulo')
            if (titulo) {
              titulo.innerHTML = 'Lo sentimos, Puzzle no resuelto.<br/>Prueba otra vez <img width="50px" src="./assets/icons/icons8-double-down-80.png" alt="Flecha abajo"/>'
              titulo.style = 'animation:feliz 3s forwards; z-index:3; position:relative; color:white; text-shadow: 2px 2px #808080, 6px 6px black;'
            }
            
            if (cajasContainer) {
              cajasContainer.style = 'background-color:#000000;transform:scale(1);gap:0;pointer-events:none'
            }
          }, 5000)
        }
      }
  ...
  function initHistoria() {
    ...
    const distanciaHis1 = () => window.innerHeight * 0.35 + (tarjeta1?.offsetHeight || 0)
    ...
    const navChangeHistoria = () => {
      ...
      if (window.innerHeight * 0.1 < window.scrollY && d1 > window.scrollY) {
        utils.qs('#tarjetaHist1 >img')?.setAttribute('style', 'animation: aparecer1 1.5s ease-out forwards;')
        utils.qs('#tarjetaHist1 >div')?.setAttribute('style', 'animation: opacity1 2s ease-out forwards')
        return
      }

      // Animación de la segunda tarjeta
      if (d1 < window.scrollY && d1 + t2h > window.scrollY) {
        utils.qs('#tarjetaHist2 >img')?.setAttribute('style', 'animation: aparecer2 1.5s ease-out forwards;')
        utils.qs('#tarjetaHist2 >div')?.setAttribute('style', 'animation: opacity2 2s ease-out forwards')
        return
      }

      // Animación de la tercera tarjeta
      if (d1 + t2h < window.scrollY) {
        utils.qs('#tarjetaHist3 >img')?.setAttribute('style', 'animation: aparecer1 1.5s ease-out forwards;')
        utils.qs('#tarjetaHist3 >div')?.setAttribute('style', 'animation: opacity1 2s ease-out forwards')
      }
    }
    ...
    // Mostrar duración inicial del video
    if (showTime) {
      setTimeout(() => { 
        showTime.innerHTML = '<b>Duración del video:</b> 04:41' 
      }, 100)
    }
  ...
  function boot() {
    ...
    const isPrueba = !!(utils.qs('#cajasoltar') || utils.qs('#cajasoltar2') || utils.qs('#cajasoltar3'))
    const isHistoria = !!(utils.qs('#tarjetaHist1') || utils.qs('#tarjetaHist2') || utils.qs('#tarjetaHist3') || utils.qs('video'))
